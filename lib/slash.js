const { isArray, each, get, set, forIn, isObject, isNumber } = require('lodash')
...
class Slash {
  ...
  /**
   * Returns the arguments passed with the slash command as an array, or one argument identified by its index
   * @param   {Number}          [idx] - The index of one of the arguments
   * @returns {String[]|String}
   */
  args(idx = null) {
    const _args = this.text.split(' ')
    return isNumber(idx)
      ? get(_args, idx)
      : _args
  }
