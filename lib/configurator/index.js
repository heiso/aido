...
async function configurator(app, dev) {
  ...
  const checklist = [{
    ...
    hints: [
      ...
      chalk`-> Take the {bold App ID} from your app's URL ({inverse https://api.slack.com/apps/APP_ID_HERE })`,
      chalk`-> And copy it in your aido config as appId`,
    ],
  }, {
    ...
    hints: [
      ...
      chalk`✍ Copy the {bold Verification Token} in your configuration as slackVerificationToken`,
    ],
  }, {
    ...
    doneMessage: 'Access tokens seem to work',
    done: async () => {
      const { body } = await customApi('api.test', {}, true, options.appToken)

      return body.ok
    },
    hints: () => [
      chalk`⚠️ Your Oauth tokens are invalid`,
    ...
    doneMessage: 'API Scopes are setup',
      ...
      const { headers, body } = await customApi('api.test', {}, true, options.appToken)
    ...
    hints: () => [
      chalk`⚠️ Aido can inspect slash commands if you provide a legacy token in your configuratioon`,
